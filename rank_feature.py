...
if __name__ == '__main__':
    ...
    # 计算方差
    df_temp = df_click.groupby([ 'user_id'])['user_id_click_article_created_at_ts_diff'].std().reset_index()
    df_temp.columns = [ 'user_id', 'user_id_click_article_created_at_ts_diff_std' ]
    df_feature = df_feature.merge(df_temp, how='left')
    ...
    df_feature = df_feature.merge(df_temp, how='left')
    # 计算方差
    df_temp = df_click.groupby(['user_id'])['user_id_click_diff'].std().reset_index()
    df_temp.columns = ['user_id', 'user_id_click_diff_std']
    df_feature = df_feature.merge(df_temp, how='left')
    ...
    df_temp = df_click.groupby(['user_id']).agg(
        ...
        user_clicked_article_words_count_std=('words_count', 'std'),
    ...
    for col in device_cols:
        if col in df_click.columns:
            # 添加最后一次点击的设备信息
            df_temp = df_click.groupby(['user_id']).agg(
                **{f'user_last_{col}': (col, lambda x: x.iloc[-1])}
            ).reset_index()
            df_feature = df_feature.merge(df_temp, how='left')
            ...
            df_temp = df_click.groupby(['user_id']).agg(
                **{f'user_most_{col}': (col, get_mode)}
            ).reset_index()
            df_feature = df_feature.merge(df_temp, how='left')
            log.info(f'已添加特征: user_last_{col}')
