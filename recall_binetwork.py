...
parser.add_argument('--logfile', default='test_binetwork.log')
parser.add_argument('--batch_size', type=int, default=512)
parser.add_argument('--device', default='cuda' if torch.cuda.is_available() else 'cpu')
parser.add_argument('--test_size', type=int, default=1000, help='测试模式下的样本数')
parser.add_argument('--topk', type=int, default=100, help='召回样本数')
...
def calculate_item_similarity_batch(
    ...
    for i in range(0, len(target_items), batch_size):
        ...
        for j in range(0, len(related_items_list), chunk_size):
            ...
            # batch_interactions = batch_interactions * user_weights
    ...
    if not df_final.empty:
        ...
        if mode == 'valid':
            ...
            hitrate_40, mrr_40, hitrate_50, mrr_50 = evaluate(
                df_final[df_final['label'].notnull()], total)

            log.debug(
                f'binetwork: {hitrate_5}, {mrr_5}, {hitrate_10}, {mrr_10}, '
                f'{hitrate_20}, {mrr_20}, {hitrate_40}, {mrr_40}, '
                f'{hitrate_50}, {mrr_50}'
            )
            log.debug(
            f"标签分布: {df_final[df_final['label'].notnull()]['label'].value_counts()}"
            )           
